...
def ViewProject(request, pk):
    ...
    if models.ProjectRate.objects.filter(user=request.user.profile, project=pk).exists():
        user_cur_vote = models.ProjectRate.objects.get(
            user=request.user.profile, project=pk)
        if user_cur_vote.is_upvote:
            user_upvoted_proj = True
        else:
            user_downvoted_proj = True
    context['rem_sum_required'] = get_projects.total_target - \
        get_projects.collected_donations
...
def AllProjects(request):
    ...
    latest_five_projects = Project.objects.filter(
        is_deleted=False).order_by('-created')[:5]
    featured_five_projects = Project.objects.filter(
        is_deleted=False, is_featured=True)[:5]
...
def DeleteProject(request, pk):
    ...
    if pro > allowed_value:
        messages.error(
            request, "Project Can't Be Deleted Because The Donation Amount Is Above 25%")
        return render(request, 'projects/viewProject.html', context)
    else:
...
def update_rating(request, project_id):
    if request.method == 'POST':
        profile = request.user.profile
        project = models.Project.objects.get(id=project_id)
        user_voted_before = models.ProjectRate.objects.filter(
            user=profile.id, project=project.id).exists()
        # user_vote_on_project = 0 # just workaround to overcome 'local variable 'user_vote_on_project' referenced before assignment'
        user_vote_on_project = models.ProjectRate.objects.filter(
            user=profile.id, project=project.id).first()

        # check action type
        action_type = request.POST.get('action_type')
        if action_type == 'Vote Up':
            if not user_voted_before:  # no up
                models.ProjectRate.objects.create(
                    is_upvote=True, user=profile, project=project)
                ...
                models.ProjectRate.objects.filter(
                    user=profile.id, project=project.id).delete()
                project.total_votes -= 1
                project.total_upvotes -= 1
        else:
            if not user_voted_before:  # no down
                models.ProjectRate.objects.create(
                    is_upvote=False, user=profile, project=project)
                project.total_votes += 1
